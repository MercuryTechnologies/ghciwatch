...
/// Formats warning and error messages with GHC-compatible colors.
pub struct WarningFormatter;
...
impl WarningFormatter {
    ...
    ///
    /// This method recognizes different types of lines in GHC output and applies
    ...
    fn colorize_line(&self, line: &str, severity: Severity) -> String {
...
impl Default for WarningFormatter {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_colorize_line_number_with_pipe() {
        let formatter = WarningFormatter::new();
        let line = "  28 | import Data.Coerce (coerce)";
        let result = formatter.colorize_line(line, Severity::Warning);
    ...
    fn test_colorize_error_flag() {
        let formatter = WarningFormatter::new();
        let line = "    [-Werror]";
        let result = formatter.colorize_line(line, Severity::Error);
    ...
    fn test_colorize_multiline_message() {
        ...
        let result = formatter.colorize_message(message, Severity::Warning);
    ...
    fn test_line_number_detection() {
        ...
        let result1 = formatter.colorize_line(line1, Severity::Warning);
        assert!(result1.contains("1"));
        assert!(result1.contains("module Main"));

        let line2 = "123 | import Data.List";
        let result2 = formatter.colorize_line(line2, Severity::Warning);
        ...
        let result3 = formatter.colorize_line(line3, Severity::Warning);
        assert_eq!(result3, line3);

        let line4 = "  | no line number";
        let result4 = formatter.colorize_line(line4, Severity::Warning);
